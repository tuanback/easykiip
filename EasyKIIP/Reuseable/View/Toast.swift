...
public extension UIView {
  ...
  // MARK: [Do.Tuan] Show message at center
  func makeToastCenter(_ message: String?, image: UIImage? = nil, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
    do {
      let toast = try toastViewForMessage(message, image: image, style: style)
      showToast(toast, duration: duration, position: position, completion: completion)
    } catch ToastError.missingParameters {
      print("Error: message, title, and image are all nil")
    } catch {}
  ...
  func makeToast(with attributedText: NSAttributedString, duration: TimeInterval = ToastManager.shared.duration, position: ToastPosition = ToastManager.shared.position, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
    do {
      let toast = try toastViewForMessage(with: attributedText, style: style)
      showToast(toast, duration: duration, position: position, completion: completion)
    } catch ToastError.missingParameters {
      print("Error: message, title, and image are all nil")
    } catch {}
  ...
  func toastViewForMessage(with attributtedText: NSAttributedString?, style: ToastStyle) throws -> UIView {
    // sanity
    guard attributtedText != nil else {
      throw ToastError.missingParameters
    }
    ...
    if let message = attributtedText {
      messageLabel = UILabel()
    ...
    let wrapperWidth = max(style.horizontalPadding * 2.0, longerX + longerWidth + style.horizontalPadding)
    let wrapperHeight = max(messageRect.origin.y + messageRect.size.height + style.verticalPadding, style.verticalPadding * 2.0)
  ...
  func toastViewForMessage(_ message: String?, image: UIImage?, style: ToastStyle) throws -> UIView {
    // sanity
    ...
    if let message = message {
      messageLabel = UILabel()
      messageLabel?.numberOfLines = style.titleNumberOfLines
      messageLabel?.font = style.titleFont
      messageLabel?.textAlignment = style.titleAlignment
      messageLabel?.lineBreakMode = .byTruncatingTail
    ...
    if let messageLabel = messageLabel {
      messageRect.origin.x = imageRect.origin.x
      messageRect.origin.y = style.verticalPadding + imageRect.origin.y + imageRect.size.height
      messageRect.size.width = messageLabel.bounds.size.width
      messageRect.size.height = messageLabel.bounds.size.height
    }
    
    let longerWidth = max( messageRect.size.width, imageRect.size.width)
    
    if messageRect.size.width > imageRect.size.width {
      let delta = (messageRect.size.width - imageRect.size.width) / 2
      imageRect.origin.x = style.horizontalPadding + delta
    }
    else {
      let delta = (imageRect.size.width - messageRect.size.width) / 2
      messageRect.origin.x = imageRect.origin.x + delta
    }
    
    let wrapperWidth = longerWidth + style.horizontalPadding * 2.0
...
public struct ToastStyle {
   ...
   */
  public var backgroundColor: UIColor = UIColor.appSecondaryBackground
  
  /**
   The title color. Default is `UIColor.whiteColor()`.
   */
  public var titleColor: UIColor = .appLabelBlack
  
  /**
   The message color. Default is `.white`.
   */
  public var messageColor: UIColor = .appLabelBlack
